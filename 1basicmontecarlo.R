...
mlw <- max(logw) + log(mean(exp(logw - mlw)))
...
gtrace1 <- qplot(x=1:100, y=chain[1, 1:100], geom="blank") +
			geom_line() +
			geom_point(aes(y=proposal_history[1, 1:100]), colour=colors[1]) +
			geom_point(aes(y=chain[1, 1:100]), colour=colors[2])
ghist1 <- qplot(x=chain[1, 100:nmcmc], geom="blank") +
			geom_histogram(aes(y=..density..), fill=colors[2]) +
			stat_function(fun=dnorm)
gtrace2 <- qplot(x=1:100, y=chain[2, 1:100], geom="blank") +
			geom_line() +
			geom_point(aes(y=proposal_history[2, 1:100]), colour=colors[1]) +
			geom_point(aes(y=chain[2, 1:100]), colour=colors[2])
ghist2 <- qplot(x=chain[2, 100:nmcmc], geom="blank") +
			geom_histogram(aes(y=..density..), fill=colors[2]) +
			stat_function(fun=dnorm)
g <- gridExtra::grid.arrange(gtrace1, ghist1, gtrace2, ghist2,
                            ncol=2, nrow=2,
