...
## inverse cdf p
inversecdfp <- function(z) uniroot(function(x) cdfp(x) - z,
                                   interval=c(0, 1))$root
## inverse cdf q
inversecdfq <- function(z) uniroot(function(x) cdfq(x) - z,
                                   interval=c(0, 1))$root
...
gotmap <- qplot(x=xseq, geom="blank") +
            geom_line(aes(y=otmap_seq))
gotmap
